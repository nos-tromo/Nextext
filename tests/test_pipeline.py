...
def test_get_api_key_prompts_and_saves(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, disable_docker_env: None) -> None:
...
def test_get_api_key_docker_environment_raises(monkeypatch: pytest.MonkeyPatch, enable_docker_env: None) -> None:
    monkeypatch.setattr(pipeline, "find_dotenv", lambda: "")
    monkeypatch.setattr(pipeline, "load_dotenv", lambda path: None)
...
def test_transcription_pipeline_invokes_transcriber(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_transcription_pipeline_falls_back_to_original_language(monkeypatch: pytest.MonkeyPatch) -> None:
    class DummyTranscriber:
...
def test_translation_pipeline_returns_input_when_language_matches(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_translation_pipeline_translates_each_row(monkeypatch: pytest.MonkeyPatch) -> None:
    class DummyTranslator:
        def __init__(self) -> None:
...
def test_summarization_pipeline_rejects_empty_text(ollama_pipeline: None = None) -> None:
...
def test_wordlevel_pipeline_invokes_all_steps(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    counts, entities, sentiments, graph, wordcloud = pipeline.wordlevel_pipeline(df, "en")
...
def test_hatespeech_pipeline_no_results_keeps_dataframe(monkeypatch: pytest.MonkeyPatch) -> None:
    class DummyDetector:
        def __init__(self, ollama_pipeline) -> None:
            pass
