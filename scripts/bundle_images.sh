...
# YYYY-MM-DD plus short git sha; override by exporting NEXTEXT_VERSION beforehand.
export NEXTEXT_VERSION="${NEXTEXT_VERSION:-$(date +%Y-%m-%d)-$(git rev-parse --short HEAD)}"
...
#   built  = local-only names like "nextext-backend-cpu" (already tagged by build)
...
if (( ${#built[@]} > 0 )); then
  docker save "${built[@]}" | gzip > "nextext-built-${PROFILE}-${NEXTEXT_VERSION}.tar.gz"
fi

if (( ${#pulled[@]} > 0 )); then
  docker save "${pulled[@]}" | gzip > "nextext-pulled-${PROFILE}-${NEXTEXT_VERSION}.tar.gz"
fi

echo "Wrote: nextext-built-${PROFILE}-${NEXTEXT_VERSION}.tar.gz, nextext-pulled-${PROFILE}-${NEXTEXT_VERSION}.tar.gz"
