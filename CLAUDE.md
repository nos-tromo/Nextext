...
**Playback.** The SPA has a slide-in media player (`frontend/src/components/player/MediaPanel.tsx`, mounted once in the Shell, driven by `lib/mediaPlayerStore.ts`). Clicking a timestamp in the Transcript or Visual context tab opens it and seeks there; the row under the playhead is highlighted and scrolled into view as playback advances (`hooks/useFollowActiveRow.ts`), so the highlight cannot drift off the fold. It scrolls only when the row has actually left the viewport, and centres it so the surrounding lines come along. Following pauses while the reader scrolls the page by hand — but not for a wheel over the player panel, nor for Space/arrows aimed at a control — and resumes on either signal: a timestamp click (the store's `followSeq` counter) or the playhead's row coming back into view. That second signal is load-bearing: a one-way pause left following dead for the rest of the session after a single trackpad flick. It is non-modal by design — no backdrop, no `aria-modal` — and the canvas reserves room for it on wide screens, so the page stays usable. `TranscriptSegment` carries `start_seconds`/`end_seconds` (numeric twins of the `str(timedelta)` display strings) so the client seeks without re-implementing the parser.
...
- `NEXTEXT_VISUAL_SUMMARY` / `VISUAL_SUMMARY_MAX_FRAMES` / `VISUAL_SUMMARY_IMAGE_MAX_SIDE` (backend + CLI) —
  ...
  sampled and downloadable as `keyframes.zip`. It is the **operator** half of a
  two-part gate; the per-job `visual_context` option (default `true`) is the
  client half, for a caller that wants the frames but not the words. Its
  default cannot be `false`: `JobOptions` forbids extra fields, so a client
  that predates the option cannot send it, and any other default would change
  what the SPA already gets. Either gate off means `_ensure_inference()` is
  never reached — which matters beyond cost, since that call sits outside the
  captioning fail-soft guard and the keyframe stage runs first, so it was the
  one thing that could fail a frames-only job on an unhealthy chat router and
  discard its transcript. Requires a vision-capable
