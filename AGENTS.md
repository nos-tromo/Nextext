...
| Translation | `nextext/core/translation.py` → `Translator`, `translation_pipeline` | Transcript `pd.DataFrame`, source and target ISO codes, inference provider | `pd.DataFrame` with an added `translation` column (segment-wise), `text` left untouched for cross-referencing | CLI `--task translate`, React UI task option |
...
- **Responsibilities:** Detect the transcript language via `langdetect` when needed, prompt the shared chat model via the inference service, and write each transcript segment's translation to the requested target language alongside the original text.
...
- **Outputs:** A new `translation` column holding the translated text; the original `text` column is left untouched so the transcript and translation can be cross-referenced side by side in the output table (CSV/XLSX exports and the UI transcript table). `Translator.src_lang` is populated for logging and downstream toggles. Downstream agents (word-level analysis, summarization, hate-speech detection) read whichever column `nextext.pipeline.effective_text_column()` resolves to — `translation` when present, else `text` — so they keep analyzing the target-language content.
...
- **Operational notes:** This is the only translation path — Whisper's audio-translate task is no longer used — so English targets are translated here too. Translation is skipped only when the resolved source language already equals the target (see `should_translate()` in `nextext/pipeline.py`); in that case no `translation` column is added. Translation runs on `TEXT_MODEL` — the same model used for summarization — over the templated prompt in `nextext/utils/prompts/en/translation.txt` (translation is not localized — its output follows the target-language placeholders) plus a translation system prompt, identically for every `INFERENCE_PROVIDER`.
...
- **Inputs:** `pd.DataFrame` with `text` (and `translation` when translation has run — analyzed instead of `text`, via `nextext.pipeline.effective_text_column()`), resolved language code (source for `transcribe`, target for `translate`).
...
- **Inputs:** Transcript `pd.DataFrame` (the `translation` column when translation has run, otherwise `text` — via `nextext.pipeline.effective_text_column()`), shared `InferencePipeline`, optional `max_chars` limit (default 2048).
