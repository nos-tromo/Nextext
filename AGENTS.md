...
| Inference Service | `nextext/modules/openai_cfg.py` → `InferencePipeline` | Prompt template + runtime options | Raw string from an OpenAI-compatible chat endpoint | Shared dependency for translation and summary agents |
...
- **Dependencies:** OpenAI-compatible chat completions via Ollama by default (`OPENAI_API_BASE`, defaults to `http://localhost:11434/v1`), or the hosted OpenAI API when `INFERENCE_PROVIDER=openai`; prompt language controlled by `InferencePipeline.out_language`.
